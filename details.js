class CookieDashboard {
    ...
    processData(data) {
        ...
        // Tìm các tập hợp con
        const bothCookieNames = new Set([...declaredCookieNames].filter(name => realtimeCookieNames.has(name)));
        const onlyDeclaredNames = new Set([...declaredCookieNames].filter(name => !realtimeCookieNames.has(name)));
        const onlyRealtimeNames = new Set([...realtimeCookieNames].filter(name => !declaredCookieNames.has(name)));
    ...
    updateStatistics() {
        ...
        document.getElementById('total-violations').textContent = this.data.total_issues || 0;
        document.getElementById('compliance-score-stat').textContent = `${this.data.compliance_score || 0}/100`;
        document.getElementById('declared-cookies').textContent = this.data.policy_cookies_count || 0;
        document.getElementById('realtime-cookies').textContent = this.data.actual_cookies_count || 0;
    }
    ...
    updateHeader(data) {
        ...
        if (data.analysis_date) {
            ...
            document.getElementById('analysis-date').textContent = `Phân tích lúc: ${date.toLocaleString('vi-VN')}`;
        }
    }
    ...
    createCookieRow(cookie, index) {
        ...
        row.innerHTML = `
            ...
            <div class="cell purpose-col">${cookie.declared ? this.escapeHtml(cookie.declared.declared_purpose || '-') : '-'}</div>
            <div class="cell third-party-col">${cookie.declared ? this.escapeHtml(cookie.declared.declared_third_parties?.join(', ') || '-') : this.escapeHtml(cookie.realtime?.domain || '-')}</div>
            <div class="cell retention-col">${cookie.declared ? this.escapeHtml(cookie.declared.declared_retention || '-') : this.formatExpiration(cookie.realtime)}</div>
            <div class="cell violation-col">
                <span class="violation-count ${cookie.violations.length > 0 ? 'has-violations' : 'no-violations'}">
    ...
    createViolationsSection(violations) {
        const violationItems = violations.map(violation => `
            <div class="violation-item">
                ...
                <div class="violation-title">Vi phạm quy tắc ${this.escapeHtml(violation.issue_id || 'N/A')}</div>
